...
ignored_files = [".env", ".gitignore", "database.db", "package-lock.json", "index.html", ]
...
def init_markdown_content():
    ...
    initial_information = "## initial information:\n" + \
        ...
        "- I am providing you with all files we already created for the project\n\n\n"
...
def generate_prompt_context(project_path):
    ...
    for root, dirs, files in os.walk(project_path):
        ...
        # Remove ignored file types from the lists
        files[:] = [f for f in files if not any(f.endswith(file_type) for file_type in ignored_file_types)]
